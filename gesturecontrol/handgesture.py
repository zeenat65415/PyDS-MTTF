    ...
    while cam.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                data2 = hand_landmarks.landmark[12]
                coords1 = mp_drawing._normalized_to_pixel_coordinates(data.x, data.y, width, height)
                coords2 = mp_drawing._normalized_to_pixel_coordinates(data2.x, data2.y, width, height)
                cv2.circle(image, coords1, 20, (0, 255, 255), -1)
                cv2.circle(image, coords2, 20, (255, 0, 255), -1)
